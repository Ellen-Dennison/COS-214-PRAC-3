...
int main()
{
    ...
    Iterator* i1 = c1->createIterator();
    std::cout <<"size " << i1->size() << std::endl;
    std::cout <<"first element " << i1->first()->getName() << std::endl;
    std::cout <<"next element " << i1->next()->getName() << std::endl;
    std::cout << "boolean value for end() " << i1->end() << std::endl; 
